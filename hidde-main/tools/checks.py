...
async def has_perms(ctx: commands.Context, perm: str=None): 
    ...
    for r in ctx.author.roles:
        ...
        if perm in [str(p[0]) for p in r.permissions if p[1] is True]: 
            return True 
        # Check custom or overridden permissions in the database
        check = await ctx.bot.db.fetchrow("SELECT permissions FROM fake_permissions WHERE role_id = $1 and guild_id = $2", r.id, ctx.guild.id)
        if check is None: continue 
        permissions = json.loads(check[0])
        if perm in permissions or "administrator" in permissions: 
            return True
    # If no permissions matched, log the missing permission for debugging
    ctx.bot.logger.warn(f"Missing permission: {perm} for user {ctx.author.id} in guild {ctx.guild.id}")
    return False 